...
class general_ledger_xls(report_xls):
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        c_specs = [
            ...
            ('project', 1, 0, 'text', _('Proyecto'), None, c_hdr_cell_style),
        ...
        for account in objects:
            #TODO SKIP LEGDGER LINES EVO

            if _p.get("acumulado_fecha"):

                skip = True
                if _p['ledger_lines'][account.id]:
                    for line in _p['ledger_lines'][account.id]:

                        if not _p.start_date or (line["ldate"] >= _p.start_date and line["ldate"] <= _p.stop_date):
                            skip = False
                            continue
                if skip:
                    continue
            ...
            if _p.display_account_raw(data) == 'all' or \
                ...
                for line in _p['ledger_lines'][account.id]:
                    ...
                    c_specs += [
                        ...
                        ('project', 1, 0, 'text', line.get('project', "")),
