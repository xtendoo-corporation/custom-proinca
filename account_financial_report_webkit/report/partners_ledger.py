...
class PartnersLedgerWebkit(report_sxw.rml_parse,
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if initial_balance_mode == 'initial_balance':
            # todo evo ledger initial balance

            year_code = datetime.today().year
            if int(start_period.fiscalyear_id.code) < year_code or self.pool.get("account.move").search(self.cr, self.uid, [('period_id.special', '=', True),('period_id.fiscalyear_id', '=', start_period.fiscalyear_id.id)]):
                opening_period_selected = self.get_included_opening_period(
                    start_period)
                initial_balance_lines = self._compute_partners_initial_balances(
                    accounts, start_period, partner_ids,
