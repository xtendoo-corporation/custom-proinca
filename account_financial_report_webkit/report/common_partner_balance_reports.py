...
class CommonPartnerBalanceReportHeaderWebkit(CommonBalanceReportHeaderWebkit,
    ...
    def _get_partners_initial_balances(self, account_ids, start_period,
        ...
        if initial_balance_mode == 'opening_balance':
            ...
            #TODO fix initial balance to evo ledger
        elif initial_balance_mode == 'initial_balance':

            year_code = datetime.today().year
            if int(start_period.fiscalyear_id.code) < year_code or \
                    self.pool.get("account.move").search(self.cr, self.uid, [('period_id.special', '=', True),('period_id.fiscalyear_id', '=', start_period.fiscalyear_id.id)]):
                opening_period_selected = self.get_included_opening_period(
                    start_period)
                res = self._compute_partners_initial_balances(
                    account_ids, start_period, partner_filter_ids,
                ...
                force_period_selected = self.pool.get('account.period').search(self.cr, self.uid, [
                    ('fiscalyear_id.code', '=', str(year_code - 1))])

                res = self._compute_partners_initial_balances(
                    account_ids, start_period, partner_filter_ids,
                    force_period_ids=force_period_selected,
                    exclude_reconcile=exclude_reconcile)
        else:
            res = {}
        return res
