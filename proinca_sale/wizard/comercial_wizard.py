...
class InvoicingCanonWizard(models.TransientModel):
    ...
    def create_canon_invoices(self):
        ...
        for contract in self.contracts:
            partners_inv_lines[contract.comercial_id.partner_id] = [[],None]

        for contract in self.contracts:
            pack_lines = self.env["sale.order.franchise.line"].search(
                [('order_id', '=', contract.id), ('franchise', '=', True)])

            for line in pack_lines:
                product = line.service_id.product_canon_id
                partners_inv_lines[contract.comercial_id.partner_id][0].append((0, 0, {
                    'name': "%s: %s" % (contract.number,line.name),
                    'qty': 1,
                    'price_unit': line.canon_amount,
                    'product_id': product.id,
                    'account_id': product._get_account_id('income', 1),
                    'invoice_line_tax_id': [(6, 0, product._get_taxes_id('income', 1))]
                }))
                partners_inv_lines[contract.comercial_id.partner_id][1] = contract
            # contract.state = 'done'
        ...
        for partner in partners_inv_lines.keys():
            ...
            partners_inv_lines[contract.comercial_id.partner_id][1].invoice_canon_id = inv_id
